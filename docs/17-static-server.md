...
`GET /` from `examples/server/www/` answered by ~140 lines of WAT across two modules —
...
```
  ...
  net.* syscalls         +--------------------------------------
  (host, swappable)      +------------------ libs/http/http.wasm
...
`tools/serve.py` did the same job in Python and is retired (kept as reference).
...
- `libs/http/http.wat` → `libs/http/http.wasm` (~1.7 KB): the hand-written lib.
- `libs/sha256/` → `libs/sha256/sha256.wasm` (63 KB): finished crates.io `sha2`,
...
./host-rs/target/release/host-rs examples/server/manifest.toml
```
